...
class BBoxConfig:
    ...
    def __init__(self, config_path: Optional[Path] = None):
        ...
        # Optional project base directory; when set, attempt to emit
        # collection sources relative to this path and include it in the
        # generated TOML so the server/operator knows the project root.
        self.project_basedir: Optional[Path] = None
    ...
    def add_tileset(self, name: str, source: Path, format: str = "png",
        ...
        # 1) If a project base dir was explicitly provided, prefer to
        #    relative_to that so generated config references project-local
        #    files (avoids data duplication).
        if self.project_basedir is not None:
            try:
                relative_source = source.relative_to(self.project_basedir)
            except Exception:
                relative_source = relative_source

        # 2) Try QGIS project dir if available and project_basedir not set
        ...
        # 3) Fall back to config/ or bbox root relativeization
        if relative_source == source:
            try:
    ...
    def add_collection(self, name: str, source: Path, srs: str = "EPSG:4326"):
        ...
        # Next try QGIS project dir if available
        if relative_source == source:
            try:
                from qgis.core import QgsProject
                proj_file = QgsProject.instance().fileName()
        ...
        # Finally try config/ or bbox root
        if relative_source == source:
            try:
                relative_source = source.relative_to(self.config_path.parent)
            except ValueError:
    ...
    def generate_toml(self) -> str:
        ...
        # Emit optional project base dir so server operators know project root
        if self.project_basedir is not None:
            lines.append(f'project_basedir = "{str(self.project_basedir).replace('\\', '/')}"')
            lines.append("")
