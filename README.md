...
- 基本方針: WMS はクライアントから渡された `ANGLE` パラメータを受け取り、サーバ側のレンダラ（QgsMapSettings）に回転角を設定してそのまま描画します。
   - 回転はサーバ内で一元化（centralized）され、0°も同じ経路で処理されます。レンダラ側で回転を適用した画像をそのまま返却するため、サーバは逆回転（output を north-up に補正する）を行いません。
- 利点: QGIS のレンダラが提供する高品質な回転済み描画（ラベルやシンボルの配置）をそのまま返却できます。ピクセルレベルの二重補間が発生しないため、画像の劣化やぼけを避けられます。
- クライアント側の注意点: クライアント（OpenLayers 等）は WMS に `ANGLE` を送信してサーバに描画角度を指示してください。クライアント側でページ読み込み時やリフレッシュ時に回転を強制的に0°にリセットする実装がある場合は削除してください（本リポジトリの実装では自動リセットを除去済みです）。
- ラベル表示: サーバ側で回転を適用するため、ラベルの向きや配置はサーバで処理されます。必要に応じてラベル回転補正のロジック（プラグイン内に補助関数あり）を利用してください。
