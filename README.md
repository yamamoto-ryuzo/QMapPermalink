...
**Version 2.11.0 (テーマ実装版)** - テーマ対応のWMS出力（仮想マップビューでのテーマ適用）を含むリリース
...
- 基本方針: クライアントは WMS に `ANGLE`（度）を渡してサーバに描画角度を指示します。クライアントはページ読み込み時は URL に含まれる角度を初期値として送り、その後ユーザ操作による回転は map の現在角度を優先して送信します。

- BBOX の扱い（重要）:
   - `GetMap` リクエストでは `BBOX` を必須と扱います。`BBOX` が無い、またはパースに失敗するリクエストはサーバ側でエラー（MissingParameterValue 等）として扱われ、暗黙的にプロジェクトの extent にフォールバックしません。

- ANGLE の処理フロー:
   - ANGLE が `0°`（または極めて小さい値）の場合:
      - 高速パスを使用します。要求された BBOX をそのまま map settings の extent に設定し、要求サイズ（WIDTH/HEIGHT）で直接レンダリングして PNG を返します。追加の拡張レンダリングや画像空間での逆回転・クロップ・リサンプルは行わず、処理が高速になります。
   - ANGLE が `0°` 以外の場合:
      - サーバは以下のパイプラインで north-up な PNG を生成して返します（クライアントはこの north-up PNG を受け取り、OpenLayers 等で view.rotation を設定して表示）:
         1. 要求 BBOX（A）を中心に、回転を考慮した外接 BBOX（B）を計算する。
         2. B をレンダリング（map_settings に rotation をセットして描画）し、大きな画像を生成する。
         3. 画像空間で -ANGLE の逆回転を画像中心基準で適用する。
         4. 逆回転画像の中心から、元の A に対応するピクセル矩形を中心クロップし、要求サイズにリサンプルして PNG を返す。

- 理由と利点:
   - この方式により、サーバ側で回転された視覚要素（ラベル、シンボル、ベクタスタイル）が正しく描画されつつ、クライアント側で north-up な画像を受け取り view.rotation を利用して表示することで、座標と画像の整合性を保ちます。
   - ANGLE=0 の高速パスにより、回転が無いリクエストについては余分な計算を避け、高速で軽量に応答できます。

- パフォーマンス注意:
   - ANGLE が非ゼロのリクエストは拡張レンダリングと画像操作（逆回転・クロップ・リサンプル）を行うため、メモリと時間を多く消費します。内部ではレンダリングサイズを上限（デフォルト 4096px）でクランプしていますが、高倍率や大きな画像サイズのリクエストは注意してください。
   - 必要に応じて追加の上限（拡大率の上限や総ピクセル数の制限）を設定することを検討してください。

- テスト例（QGIS 環境で実行）:
   - ANGLE=0（高速パス、期待: north-up PNG）:
      ```
      ...
      ```
   - ANGLE=30（拡張レンダ→逆回転→クロップ→リサンプル）:
      ```
      http://127.0.0.1:8089/wms?SERVICE=WMS&REQUEST=GetMap&VERSION=1.3.0&CRS=EPSG:3857&BBOX=<minx,miny,maxx,maxy>&WIDTH=800&HEIGHT=600&FORMAT=image/png&ANGLE=30
      ```
   - BBOX 欠如（期待: エラー応答）:
      ```
      http://127.0.0.1:8089/wms?SERVICE=WMS&REQUEST=GetMap&VERSION=1.3.0&CRS=EPSG:3857&WIDTH=800&HEIGHT=600&FORMAT=image/png
      ```
...
## リリースノート: Version 2.11.0 (テーマ実装版)

- テーマ対応のWMS出力を実装しました。`theme` パラメータで指定されたマップテーマを仮想マップビューに適用してPNGを生成します。
- 仮想マップビュー方式により、現在のプロジェクトやキャンバスを変更せずにテーマのみを適用してレンダリングが可能です。
- 複数のQGIS 3.xバージョンに対応するため、スタイルエクスポートの互換性を強化しました（exportNamedStyle の呼び出し方法にフォールバックを導入）。
