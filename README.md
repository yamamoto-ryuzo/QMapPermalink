...
#### 🚀 現行バージョンで提供している機能
-
※ 本リリース (v2.0.0) では、組み込みHTTPサーバーに WMS（Web Map Service）1.3.0 互換の配信機能を追加しました。WMS エンドポイントにより外部の GIS クライアントやブラウザから QGIS の地図を直接参照できます。開発・検証用途で外部アクセスを許可するため、サーバーは全インターフェース（0.0.0.0）にバインドされます。運用時は適切なネットワーク制御・ファイアウォール設定を行ってください。
