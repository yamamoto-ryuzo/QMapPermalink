...
- **Google Maps連携**: 同一地点のGoogle Mapsハイパーリンクを自動生成し、共有・ブラウザ確認が容易
...
- **Google Maps連携**: 応答ページに同じ地点のGoogle Mapsリンクを含む
...
2. **テーマオプション選択**: ドロップダウンから以下のいずれかを選択
   - `-- No Theme (Position Only) --`: 位置情報のみのシンプルなパーマリンク（デフォルト）
   - `-- Use Current State --`: 現在の地図状態を含む完全なパーマリンク
   - 具体的なテーマ名: 既存のマップテーマを選択してパーマリンクに含める
...
- **🎯 1つのドロップダウンで完全制御**: チェックボックスを廃止し、全ての選択肢を統合
- **📋 明確な選択肢**:
  - `-- No Theme (Position Only) --`: 位置情報のみ（軽量・高速）
  - `-- Use Current State --`: 現在のマップテーマ、レイヤーの表示状態、透明度、スタイルを全て含む
  - 具体的なテーマ名: プロジェクト内の既存マップテーマを活用
- **🔄 動的テーマリスト**: プロジェクト内の利用可能なテーマを自動検出・表示
...
- **現在バージョン**: V1.5.0（統合テーマ制御・既存テーマ選択機能）
