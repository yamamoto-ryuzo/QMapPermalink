...
2. **テーマオプション選択**: 「Include current theme/layer states」チェックボックスでテーマ情報の有無を選択
3. **パーマリンク生成**: 「Generate Permalink」ボタンで現在の地図ビューのリンクを生成
...
- **チェックボックスON**: マップテーマ、レイヤーの表示状態、透明度、スタイルを全て含む完全なパーマリンク
- **チェックボックスOFF**: 位置情報のみのシンプルなパーマリンク（従来通り）
...
- **現在バージョン**: V1.4.0（テーマ機能対応・完全な地図状態保存復元機能）
